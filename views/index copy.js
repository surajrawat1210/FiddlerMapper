...
function FindParamsInUrlOfValue(value, key, index, call) {
    var valueList = [];
    try {
...
function processRequestCookies(call, index) {
    Object.keys(call.RequestCookie).forEach(key => {
        var value = call.RequestCookie[key].value;
        if (typeof (call.RequestCookie[key].value) == "string" && call.RequestCookie[key].value.startsWith("Bearer ")) {
            var valueArray = call.requestHeader[key].value.split(" ");
            value = valueArray[1].trim();
        }
        findDependency(value, "RequestCookie", call.RequestCookie[key].name, index, call.url);
    });
}
